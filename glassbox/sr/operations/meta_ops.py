...
def normalize_formula_ascii(formula: str) -> str:
    ...
    normalized = re.sub(
        r"√\|([^|]+)\|/\|([^|]+)\|", r"sqrt(abs(\1))/abs(\2)", normalized
    )
    normalized = re.sub(
        r"([A-Za-z0-9_()]+)\s*/√\|([^|]+)\|", r"\1/sqrt(abs(\2))", normalized
    )
...
class MetaPeriodic(nn.Module):
    ...
    def snap_to_discrete(
        self, omega_threshold: float = 0.2, phi_threshold: float = 0.15
    ):
...
class MetaArithmeticExtended(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        beta_val = (
            self.beta.item() if isinstance(self.beta, nn.Parameter) else self.beta
        )
        gamma_val = (
            self.gamma.item() if isinstance(self.gamma, nn.Parameter) else self.gamma
        )
        ...
        res_sub = x - y
        res_mul = x * y
        res_div = x / torch.sqrt(
            1.0 + torch.square(y)
        )  # Soft division matching C++ eval.h
        ...
        for name, param in module.named_parameters():
            ...
            for i in range(flat.numel()):
                ...
                if const_name is not None:
                    ...
                    if verbose:
                        print(
                            f"Snapped {key}: {val:.6f} -> {const_name} ({snapped_val:.6f})"
                        )
...
class ConstantAwareLinear(nn.Linear):
    ...
    def snap_to_constants(
        self, threshold: float = 0.05, verbose: bool = False
    ) -> dict[str, str]:
