...
class ONNVisualizer:
    ...
    def _setup_figure(self, figsize):
        ...
        if self.lite_mode:
            ...
            self.ax_fitness.set_title(
                "Training Progress", color=COLORS["primary"], fontsize=12
            )

            # Formula display (center)
            self.ax_formula = self.fig.add_subplot(gs[0, 1])
            self.ax_formula.set_facecolor(COLORS["background"])
            self.ax_formula.set_title(
                "Best Formula", color=COLORS["primary"], fontsize=12
            )
            ...
            self.ax_fit.set_title(
                "Prediction vs Target", color=COLORS["primary"], fontsize=12
            )
        else:
            ...
            self.ax_network.set_title(
                "Network Architecture", color=COLORS["primary"], fontsize=14
            )
    ...
    def _draw_network(self):
        ...
        LOW_IMPORTANCE = (
            0.05  # Below this = dead (red), between = low importance (blue)
        )
        ...
        for layer_idx in range(self.n_layers):
            ...
            for node_idx, y in enumerate(hidden_y):
                ...
                imp = (
                    node_importance.get(key, 0.0) / max_importance
                    if max_importance > 0
                    else 0.0
                )
        ...
        for key, (x, y) in node_positions.items():
            ...
            imp = (
                node_importance.get(key, 0.1) / max_importance
                if max_importance > 0
                else 0.1
            )
    ...
    def _draw_connections(self, ax, node_positions, layer_x):
        ...
        for layer_idx in range(self.n_layers + 1):
            if layer_idx == 0:
                ...
                # Hidden to hidden
                from_keys = [
                    ("hidden", layer_idx - 1, i) for i in range(self.nodes_per_layer)
                ]
                to_keys = [
                    ("hidden", layer_idx, i) for i in range(self.nodes_per_layer)
                ]
                is_to_output = False
            else:
                # Last hidden to output
                from_keys = [
                    ("hidden", self.n_layers - 1, i)
            ...
            for from_key in from_keys:
                for to_key in to_keys:
                    if from_key in node_positions and to_key in node_positions:
                        ...
                        connections.append(
                            ((x1, y1, x2, y2), weight, is_to_output, from_key, to_key)
                        )
    ...
    def _get_connection_weights(self) -> dict:
        ...
        try:
            ...
            if hasattr(self.current_model, "output_proj"):
                output_weights = (
                    self.current_model.output_proj.weight.data[0].cpu().numpy()
                )

                # Normalize for visualization
                max_weight = max(
                    abs(output_weights.max()), abs(output_weights.min()), 1e-6
                )
                ...
                for i in range(self.n_inputs):
                    w = (
                        float(output_weights[idx]) / max_weight
                    ...
                    if hasattr(layer, "nodes"):
                        for node_idx, node in enumerate(layer.nodes):
                            ...
                            if hasattr(node, "get_routing_info"):
                                ...
                                primary_sources = routing_info.get(
                                    "primary_sources", []
                                )
                                routing_weights = routing_info.get(
                                    "weights", [1.0, 1.0]
                                )
                                ...
                                for src_slot, src_idx in enumerate(primary_sources[:2]):
                                    ...
                                    w = (
                                        routing_weights[src_slot]
                                        if src_slot < len(routing_weights)
                                        else 0.5
                                    )
                                    weights[(from_key, to_key)] = w

                            # Also get edge/scale weights if available
                            if hasattr(node, "router") and hasattr(
                                node.router, "scales"
                            ):
    ...
    def _draw_fitness_curve(self):
        ...
        if self.best_fitness < float("inf"):
            ax.axhline(
                ...
                linestyle="--",
                alpha=0.5,
                label=f"Best: {self.best_fitness:.4f}",
            )
    ...
    def _draw_fit_plot(self):
        ...
        # Plot target
        ax.scatter(
            x_sorted, y_sorted, color=COLORS["primary"], s=20, alpha=0.6, label="Target"
        )
