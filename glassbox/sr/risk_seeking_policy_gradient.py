...
class GradientMonitor:
    ...
    def update(self, loss: float, grad_norm: float | None = None):
...
def test_gradient_monitor():
    ...
    for i in range(15):
        ...
        if i >= 12:
            print(
                f"  Iter {i}: grad_norm={grad_norm:.2f}, exploding={monitor.is_exploding()}"
            )
...
def test_risk_seeking_selection():
    ...
    for i, (f, p) in enumerate(zip(fitnesses, probs)):
        print(
            f"  Individual {i}: fitness={f:.2f}, prob={p:.4f} {'***' if p > 0.2 else ''}"
        )
