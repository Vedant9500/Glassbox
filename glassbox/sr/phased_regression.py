...
try:
    from .optimizers.bfgs_optimizer import (
...
class PhasedSymbolicRegressor:
    ...
    def phase2_pure_basis_regression(
        ...
        features_with_bias = torch.cat(
            [features, torch.ones(n_samples, 1, device=self.device)], dim=1
        )
    ...
    def phase2_coefficient_refinement(
        ...
        features_with_bias = torch.cat(
            [features_normalized, torch.ones(n_samples, 1, device=self.device)], dim=1
        )
        ...
        bias_adjustment = (
            weights_normalized[:-1] * feature_means.squeeze() / feature_stds.squeeze()
        ).sum()
    ...
    def phase3_iterative_refinement(
        ...
        for iteration in range(n_iterations):
            ...
            X = torch.cat(
                [active_features, torch.ones(n_samples, 1, device=self.device)], dim=1
            )
            ...
            if len(active_names) > 1:
                ...
                if verbose:
                    print(
                        f"  Removing: {active_names[weakest_idx]} (weight={weights[weakest_idx].item():.4f})"
                    )
...
def test_phased_regression():
    ...
    x, y, _ = generate_polynomial_data(
        n_samples=300, formula="sin(π)+x^2", noise_std=0.02
    )
    ...
    for x_val in test_points:
        ...
        y_formula = 1.0 * x_val**2 + 1.0 * np.sin(
            x_val
        )  # Using discovered coefficients
