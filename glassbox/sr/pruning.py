...
class PostTrainingPruner:
    ...
    def sensitivity_analysis(
            ...
            for layer_idx, layer in enumerate(self.model.layers):
                for node_idx, node in enumerate(layer.nodes):
                    ...
                    if hasattr(node, "output_proj"):
                        ...
                        orig_bias = (
                            node.output_proj.bias.data.clone()
        ...
        if hasattr(self.model, "output_proj"):
            ...
            for ch_idx in range(n_channels):
                ...
                if verbose and abs(orig_ch) > 0.01:
                    ...
                    print(
                        f"  {ch_name} (w={orig_ch:.3f}): Δ={importance:+.4f} ({status})"
                    )
        ...
        if verbose:
            ...
            important = sum(
                1 for s in self.sensitivity_scores.values() if 0.1 < s <= 1.0
            )
    ...
    def gradient_sensitivity_analysis(self, verbose: bool = True) -> dict[str, float]:
                ...
                for node_idx, node in enumerate(layer.nodes):
                    if hasattr(node, "output_proj"):
                        ...
                        if w.grad is not None:
                            ...
                            self.sensitivity_scores[f"L{layer_idx}_N{node_idx}"] = (
                                importance
                            )
        ...
        if verbose:
            print(
                f"Computed sensitivity for {len(self.sensitivity_scores)} components via gradients."
            )
    ...
    def recursive_graph_prune(
        ...
        if verbose:
            print(
                f"Dead nodes (importance ≤ {importance_threshold}): {len(dead_nodes)}"
            )
    ...
    def mask_and_finetune(
            ...
            for layer_idx, layer in enumerate(self.model.layers):
                for node_idx, node in enumerate(layer.nodes):
                    if hasattr(node, "output_proj"):
                        ...
                        mask = (weight.abs() >= weight_threshold).float()
                        masks[
                            f"layers.{layer_idx}.nodes.{node_idx}.output_proj.weight"
                        ] = mask
                        total_weights += weight.numel()
                        masked_weights += (mask == 0).sum().item()
        ...
        if verbose:
            ...
            print(
                f"Masked {masked_weights}/{total_weights} weights ({sparsity:.1f}% sparsity)"
            )
        ...
        if output_params and finetune_steps > 0:
            ...
            def apply_masks():
                with torch.no_grad():
                    if (
                        hasattr(self.model, "output_proj")
                        and "output_proj.weight" in masks
                    ):
                        self.model.output_proj.weight.data.mul_(
                            masks["output_proj.weight"]
                        )
                    ...
                    try:
                        ...
                        if formula and formula != "0":
                            ...
                            if hasattr(self.model, "output_proj"):
                                ...
                                weight = 1.0  # Default
                                node_formulas[formula].append(
                                    (layer_idx, node_idx, weight)
                                )
                    except Exception as e:
                        if verbose:
                    ...
                    if layer_idx < len(self.model.layers):
                        ...
                        if node_idx < len(layer.nodes):
                            ...
                            if hasattr(node, "output_proj"):
                                ...
                                primary_node_obj = self.model.layers[
                                    primary_layer
                                ].nodes[primary_node]
                                if hasattr(primary_node_obj, "output_proj"):
                                    with torch.no_grad():
                                        # Add weights
                                        primary_node_obj.output_proj.weight.data.add_(
                                            node.output_proj.weight.data
                                        )
                                        # Add bias if both exist
                                        if (
                                            primary_node_obj.output_proj.bias
                                            is not None
                                            and node.output_proj.bias is not None
                                        ):
                                            primary_node_obj.output_proj.bias.data.add_(
                                                node.output_proj.bias.data
                                            )
                                ...
                                if verbose:
                                    print(
                                        f"  Merged L{layer_idx}_N{node_idx} into L{primary_layer}_N{primary_node}"
                                    )
    ...
    def iterative_backward_prune(
        ...
        for iteration in range(max_iterations):
            ...
            # Fine-tune
            self.mask_and_finetune(
                weight_threshold=0.0, finetune_steps=100, verbose=False
            )
            ...
            if current_mse > max_acceptable_mse:
                if verbose:
                    print("  MSE exceeded tolerance, stopping")
                break
    ...
    def prune_full_pipeline(
        ...
        self.iterative_backward_prune(
            mse_tolerance=mse_tolerance, min_importance=0.05, verbose=verbose
        )
        ...
        self.mask_and_finetune(
            weight_threshold=0.05, finetune_steps=500, verbose=verbose
        )
        ...
        if verbose:
            ...
            improvement = (
                (initial_mse - final_mse) / initial_mse * 100 if initial_mse > 0 else 0
            )
