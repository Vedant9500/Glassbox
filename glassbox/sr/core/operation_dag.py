...
from .operation_node import OperationLayer, OperationNodeSimple
...
class OperationDAG(nn.Module):
    ...
    def __init__(
        ...
        op_constraints: dict[str, bool] | None = None,  # Operator constraints
    ):
        """
        Args:
    ...
    def get_graph_summary(self) -> str:
        ...
        lines.append(
            f"Hidden: {self.n_hidden_layers} layers × {self.nodes_per_layer} nodes"
        )
        lines.append("")
    ...
    def get_formula(self, var_names: list[str] | None = None) -> str:
        """
        ...
        for layer_idx, layer in enumerate(self.layers):
            for node_idx, node in enumerate(layer.nodes):
                ...
                src2_idx = min(
                    primary_sources[1] if len(primary_sources) > 1 else 0,
                    len(expressions) - 1,
                )
                src1_expr = (
                    expressions[src1_idx]
                    if src1_idx < len(expressions)
                    else var_names[0]
                )
                src2_expr = (
                    expressions[src2_idx]
                    if src2_idx < len(expressions)
                    else var_names[0]
                )
        ...
        output_bias = (
            self.output_proj.bias.data[0].item()
        ...
        for _, weight, idx, expr in contributions[:5]:
            ...
            elif weight_sym is not None and weight_sym not in [
                str(int(round(weight))),
                f"{weight:.2g}",
            ]:
        ...
        if abs(output_bias) > weight_threshold:
            ...
            if bias_sym is not None and bias_sym not in [
                str(int(round(output_bias))),
                f"{output_bias:.2g}",
            ]:
    ...
    def forward_compiled(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for layer_idx, layer_ops in enumerate(self._compiled_ops):
            ...
            for node_info in layer_ops:
                ...
                if (
                    batch_size > 1
                    and "output_norm" in node_info
                    and node_info["output_norm"] is not None
                ):
    ...
    def finalize_coefficients(self, x: torch.Tensor) -> "OperationDAG":
        ...
        with torch.no_grad():
            ...
            b = (
                self.output_proj.bias.data
                if self.output_proj.bias is not None
                else torch.zeros(1, device=W.device)
            )
...
def train_onn(
    ...
    for epoch in range(epochs):
        ...
        if anneal_tau:
            ...
            for layer in model.layers:
                for node in layer.nodes:
                    ...
                    if hasattr(node, "op_selector") and hasattr(
                        node.op_selector, "set_tau"
                    ):
        ...
        components = train_step(
            model, optimizer, x_train, y_train, loss_fn, hard=use_hard
        )
