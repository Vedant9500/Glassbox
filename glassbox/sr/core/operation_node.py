...
class AdaptiveArityRouter(nn.Module):
    ...
    def forward_unary(
        self, sources: torch.Tensor, tau: float = 1.0, hard: bool = False
    ) -> torch.Tensor:
...
class OperationNode(nn.Module):
    ...
    def forward(
        ...
        if len(self.binary_ops) > 1:
            ...
            binary_result = (
                binary_result
                + self.binary_ops[1](agg_input, dim=-1) * binary_weights[1]
            )
        ...
        info = {
            ...
            if hasattr(self.router, "get_primary_sources")
            else [],
        }
    ...
    def __init__(
        ...
        if use_simple_nodes:
            self.nodes = nn.ModuleList(
                [OperationNodeSimple(n_sources, node_idx=i) for i in range(n_nodes)]
            )
        else:
            self.nodes = nn.ModuleList(
                [
