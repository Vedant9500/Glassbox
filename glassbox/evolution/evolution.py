...
def set_model_tau(model: nn.Module, tau: float):
    ...
    for module in model.modules():
        ...
        if _selector_types and isinstance(module, _selector_types):
            if hasattr(module, "set_tau") and callable(module.set_tau):
                module.set_tau(tau)
            elif hasattr(module, "tau") and not isinstance(module.tau, nn.Parameter):
                module.tau = tau
            continue
...
def detect_dominant_frequency(
    ...
    try:
        ...
        harmonic_fundamental: float | None = None
        is_harmonic_series = False
        ...
        if len(detected_omegas) >= 2:
            ...
            if candidate_fund > 0.15:
                ...
                int_ratios = [
                    abs(r - round(r)) < 0.12 and round(r) >= 2 for r in ratios
                ]
...
def anneal_entropy_weight(
    generation: int,
    total_generations: int,
    start_weight: float = 0.001,
...
def progressive_round_loss(
    model: nn.Module, target_params: list[str] = None
) -> torch.Tensor:
    """
...
class StructureConfidenceTracker:
    ...
    def update(
        ...
        correlation: float,
        mse: float,
        validation_corr: float | None = None,
    ) -> float:
        """
    ...
    def _correlation_confidence(self, corr: float) -> float:
        ...
        return (corr - self.CORR_THRESHOLD_LOW) / (
            self.CORR_THRESHOLD_HIGH - self.CORR_THRESHOLD_LOW
        )
    ...
    def should_refine_only(self) -> bool:
        ...
        return (
            self.confidence >= self.REFINEMENT_ONLY_CONFIDENCE
            and self.generations_stable >= self.stability_generations
        )
...
def seed_population_from_classifier(
    ...
    if simplified:
        unary_map = {
            "sin": 0,
            "cos": 0,
            "periodic": 0,  # MetaPeriodic
            "power": 1,
            "identity": 1,
            "polynomial": 1,
            "rational": 1,  # MetaPower
        }
    else:
        unary_map = {
            "sin": 0,
            ...
            if random.random() < mutation_rate:
                if "logit" in name or "selector" in name:
                    ...
                    if random.random() < 0.3:
                        ...
                        param.fill_(
                            random.choice(
                                [0.5, 1.0, 2.0, math.pi, 3.0, 3.5, 4.0, 5.0, 6.0]
                            )
                        )
                    else:
...
def mutate_operations_lamarckian(
    individual: Individual, mutation_rate: float = 0.3
) -> Individual:
    """
...
def normalize_param_sensitivities(sensitivities: dict[str, float]) -> dict[str, float]:
    ...
    return {
        name: min(1.0, max(0.0, value / max_sens))
...
def mutate_operations_gradient_informed(
    ...
    sensitivities = _public_api_func("compute_param_sensitivity")(
        individual.model, x, y
    )
...
def refine_constants(
    ...
    for name, param in model.named_parameters():
        ...
        if scales_only:
            if (
                "output_scale" in name
                or "edge_weights" in name
            ...
            is_internal = any(
                x in name for x in [".p", ".omega", ".phi", ".beta", "amplitude"]
            )
    ...
    try:
        if use_lbfgs:
            ...
            optimizer = LBFGS(
                constant_params, lr=1.0, max_iter=steps, line_search_fn="strong_wolfe"
            )
            ...
            for step in range(steps):
                ...
                if amp_enabled and scaler is not None:
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        constant_params, max_norm=GRADIENT_CLIP_NORM
                    )
...
def quick_refine_internal(
    ...
    try:
        optimizer = LBFGS(
            internal_params, lr=1.0, max_iter=steps, line_search_fn="strong_wolfe"
        )
        ...
        for layer in model.layers:
            for node in layer.nodes:
                ...
                # Low cost: Arithmetic and identity
                    ...
                    "add" in op_str or "mul" in op_str or "+" in op_str or "*" in op_str
                ):
                    complexity += 0.5
...
def intensive_coefficient_refinement(
    ...
    if current_mse > target_mse:
        ...
        scale_params = [
            p for n, p in model.named_parameters() if "scale" in n and "alpha" not in n
        ]
        if scale_params:
            model.train()
            ...
            for step in range(max_steps // 2):
                ...
                if amp_enabled and scaler is not None:
                    ...
                    scaler.unscale_(optimizer)
                    torch.nn.utils.clip_grad_norm_(
                        scale_params, max_norm=SCALE_GRADIENT_CLIP_NORM
                    )
                    scaler.step(optimizer)
                    scaler.update()
                else:
...
def finalize_model_coefficients(
    ...
    output_params = [
        p for n, p in model.named_parameters() if "output_proj" in n and p.requires_grad
    ]
    ...
    if output_params and initial_mse > target_mse:
        try:
            # Create optimizer first, then define closure
            optimizer = LBFGS(
                output_params, lr=1.0, max_iter=max_steps, line_search_fn="strong_wolfe"
            )
            ...
            logger.debug(
                f"finalize_model_coefficients L-BFGS pass 1 failed (optimization error): {e}"
            )
        except Exception as e:
            logger.warning(
                f"finalize_model_coefficients L-BFGS pass 1 failed (unexpected): {e}"
            )

    # Prune small coefficients
    prune_small_coefficients(
        model, threshold_ratio=sparsity_threshold, absolute_threshold=0.1
    )
    ...
    if output_params and post_prune_mse > target_mse:
        try:
            ...
            logger.debug(
                f"finalize_model_coefficients L-BFGS pass 2 failed (optimization error): {e}"
            )
        except Exception as e:
            logger.warning(
                f"finalize_model_coefficients L-BFGS pass 2 failed (unexpected): {e}"
            )

    # Final pruning
    prune_small_coefficients(
        model,
        threshold_ratio=FINAL_PRUNE_THRESHOLD_RATIO,
        absolute_threshold=FINAL_PRUNE_ABSOLUTE,
    )
...
def ablate_and_select_terms(
    ...
    with torch.no_grad():
        ...
        bias = (
            model.output_proj.bias.data.clone()
    ...
    if len(active_indices) > max_terms:
        sorted_idx = sorted(
            active_indices, key=lambda i: abs(weights[i].item()), reverse=True
        )
    ...
    best_results[len(active_indices)] = (
        baseline_mse,
        model.get_formula() if hasattr(model, "get_formula") else "?",
        active_indices.copy(),
    ...
    for n_terms in range(1, len(active_indices)):
        ...
        for subset in combinations(active_indices, n_terms):
            ...
            output_params = (
                [model.output_proj.weight, model.output_proj.bias]
                if model.output_proj.bias is not None
                else [model.output_proj.weight]
            )
            try:
                ...
                optimizer = LBFGS(
                    output_params, lr=1.0, max_iter=200, line_search_fn="strong_wolfe"
                )
                optimizer.step(closure)
        ...
        if best_for_n is not None:
            ...
            if verbose:
                print(
                    f"  [{n_terms} terms] best MSE={best_for_n[0]:.6f} formula={best_for_n[1]}"
                )
...
class EvolutionaryONNTrainer(RiskSeekingEvolutionMixin):
    ...
    def __init__(
        ...
        self.best_explorer: Individual | None = None  # Best found by explorers
    ...
    def initialize_population(self, x: torch.Tensor = None, y: torch.Tensor = None):
        ...
        if self.use_curve_classifier and x is not None and y is not None:
            try:
                ...
                if classifier_predictions:
                    logger.info(
                        f"CNN seeding with predictions: {classifier_predictions}"
                    )
                    print(
                        f"🧠 CNN-seeded initialization: {list(classifier_predictions.keys())}"
                    )
        ...
        for i in range(self.population_size):
            ...
            if classifier_predictions:
                ...
                random_operation_init(
                    model, bias_strength=DEFAULT_BIAS_STRENGTH + i * BIAS_INCREMENT
                )
        ...
        if self.use_explorers:
            for i in range(self.n_explorers):
                ...
                random_operation_init(
                    model,
                    bias_strength=EXPLORER_BIAS_STRENGTH + i * EXPLORER_BIAS_INCREMENT,
                )
            ...
            if classifier_predictions:
                print(
                    f"Initialized CNN-seeded population of {self.population_size} individuals + {self.n_explorers} explorers (random)"
                )
            else:
                print(
                    f"Initialized population of {self.population_size} individuals + {self.n_explorers} explorers"
                )
        else:
        ...
        if self.visualizer is not None:
            ...
            n_layers = (
                sample_model.n_hidden_layers
                if hasattr(sample_model, "n_hidden_layers")
                else 2
            )
            nodes_per_layer = (
                sample_model.nodes_per_layer
                if hasattr(sample_model, "nodes_per_layer")
                else 4
            )
    ...
    def evaluate_fitness(
        ...
        for ind in all_individuals:
            ...
            if (
                hasattr(ind, "_is_elite")
                and ind._is_elite
                and ind.fitness < float("inf")
            ):
                continue
            ...
            try:
                with torch.no_grad():
                    ...
                    if math.isnan(mse) or math.isinf(mse):
                        ...
                        fitness = (
                            mse * (1.0 + complexity_factor)
                            if math.isfinite(mse * complexity_factor)
                            else float("inf")
                        )

                        # Coefficient sparsity penalty (Hoyer-inspired)
                        # INCREASED: Stronger sparsity encourages fewer output terms
                        sparsity = _public_api_func("coefficient_sparsity_loss")(
                            ind.model
                        ).item()
                        ...
                        if current_entropy_weight > 0 and hasattr(
                            ind.model, "entropy_regularization"
                        ):
                            try:
                                entropy = ind.model.entropy_regularization().item()
                                fitness = _add_if_finite(
                                    fitness, entropy, current_entropy_weight
                                )
                            except Exception:
                                pass
                        ...
                        if self.progressive_round_weight > 0:
                            try:
                                round_loss = _public_api_func("progressive_round_loss")(
                                    ind.model
                                ).item()
                                fitness = _add_if_finite(
                                    fitness, round_loss, self.progressive_round_weight
                                )
                            except Exception:
                                pass

                        ind.fitness = (
                            fitness if math.isfinite(fitness) else float("inf")
                        )
            except Exception:
                ind.fitness = float("inf")
    ...
    def evolve_explorers(self, x: torch.Tensor, y: torch.Tensor):
        ...
        while len(new_explorers) < self.n_explorers:
            ...
            parent = random.choice(
                sorted_explorers[: max(1, len(sorted_explorers) // 2)]
            )
            ...
            if random.random() < 0.3:
                ...
                child = mutate_operations(
                    parent, mutation_rate=self.explorer_mutation_rate
                )
    ...
    def select_and_reproduce(
        ...
        effective_rate = (
            mutation_rate if mutation_rate is not None else self.mutation_rate
        )
        x = x.to(self.device)
        y = y.to(self.device)
        ...
        if diversity < 3:
            ...
            while len(new_population) < self.population_size:
                ...
                use_rspg = (
                    ...
                    and hasattr(self, "should_use_rspg")
                    and self.should_use_rspg()
                )

                if use_rspg:
                    # Risk-seeking selection: use probabilities that favor top performers
                    fitnesses = [ind.fitness for ind in selection_pool]
                    parents = self.select_parents_rspg(
                        selection_pool, fitnesses, n_parents=1
                    )
                    parent = parents[0]
                else:
                    # Normal tournament selection
                    candidates = random.sample(
                        selection_pool, min(3, len(selection_pool))
                    )
                ...
                base_steps = (
                    self.constant_refine_steps // 2
                    if self.lamarckian
                    else self.constant_refine_steps
                )
    ...
    def train(
        ...
        x: torch.Tensor,
        y: torch.Tensor,
        fitness_x: torch.Tensor | None = None,
        fitness_y: torch.Tensor | None = None,
        generations: int = 50,
        print_every: int = 5,
        ...
        try:
            if self.normalize_data:
                raise ImportError(
                    "C++ backend disabled when normalize_data=True to preserve output-space scoring"
                )
            ...
            if x_cpu.dim() == 1 or (x_cpu.dim() > 1 and x_cpu.shape[1] == 1):
                detected_omegas = detect_dominant_frequency(
                    x_cpu, y_cpu, n_frequencies=3
                )
                if detected_omegas and detected_omegas[0] != 1.0:
                    print(
                        f"FFT detected frequencies (omega) for C++ seeding: {[f'{o:.2f}' for o in detected_omegas]}"
                    )
                else:
                    detected_omegas = []
            ...
            X_list = (
                [x_cpu[:, i].numpy() for i in range(x_cpu.shape[1])]
                if x_cpu.dim() > 1
                else [x_cpu.numpy()]
            )
            ...
            for idx, node in enumerate(result["nodes"]):
                ...
                op = (
                    ["Input", "Constant", "Unary", "Binary"][op_type]
                    if op_type < 4
                    else "Unknown"
                )
                ...
                print(
                    f"  Node {idx}: {op} (val={val:.2f}, feat={feat}) left={node.get('left_child')}, right={node.get('right_child')}"
                )
            ...
            score_mse = (
                display_mse if display_mse is not None else float(result["best_mse"])
            )
            ...
            return {
                ...
                "history": [
                    {"generation": i, "best_fitness": 0} for i in range(generations)
                ],
                "final_mse": score_mse,
        ...
        self.initialize_population(
            x_original.to(self.device), y_original.to(self.device)
        )
        ...
        if x.dim() == 1 or (x.dim() > 1 and x.shape[1] == 1):
            ...
            if detected_omegas and detected_omegas[0] != 1.0:
                print(
                    f"FFT detected frequencies (omega): {[f'{o:.2f}' for o in detected_omegas]}"
                )
                ...
                if self.use_explorers:
                    for i, explorer in enumerate(self.explorers):
                        seed_omega_from_fft(
                            explorer.model, detected_omegas, individual_idx=i
                        )
        ...
        for gen in range(generations):
            ...
            if self.use_explorers and not self.confidence_tracker.should_refine_only():
                explorer_tau = max(
                    current_tau, MIN_EXPLORER_TAU
                )  # Explorers never go below min
                for explorer in self.explorers:
            ...
            self.evaluate_fitness(
                fit_x, fit_y, generation=gen, total_generations=generations
            )
            ...
            if self.best_ever and gen >= 3:
                ...
                with torch.no_grad():
                    ...
                    y_sq = y.squeeze()
                    corr = torch.corrcoef(torch.stack([pred_sq.cpu(), y_sq.cpu()]))[
                        0, 1
                    ].item()
            ...
            # Stats
            fitnesses = [
                ind.fitness for ind in self.population if ind.fitness < float("inf")
            ]
            if fitnesses:
            ...
            if self.best_ever is not None:
                ...
                if self.stuck_generations >= 5 and diversity <= 3:
                    print(
                        f"POPULATION RESTART: Stuck for {self.stuck_generations} gens, injecting fresh individuals"
                    )
                    ...
                    while len(new_population) < self.population_size:
                        model = self.model_factory().to(self.device)
                        random_operation_init(
                            model, bias_strength=random.uniform(1.0, 5.0)
                        )
            ...
            if gen % print_every == 0 or gen == generations - 1:
                ...
                if self.use_explorers and self.explorers:
                    valid_explorer_fits = [
                        e.fitness for e in self.explorers if e.fitness < float("inf")
                    ]
                ...
                if self.gradient_monitor is not None:
                    ...
                    if rspg_stats.get("is_rspg_active", 0.0) > 0.5:
                        ...
                        explode = (
                            "EXPLODE"
                            if rspg_stats.get("is_exploding", 0.0) > 0.5
                            else ""
                        )
                ...
                conf_info = ""
                if (
                    hasattr(self, "confidence_tracker")
                    and self.confidence_tracker.confidence > 0
                ):
                    eff_rate = self.confidence_tracker.get_effective_mutation_rate(
                        self.mutation_rate
                    )
                    conf_info = f" | Conf:{self.confidence_tracker.confidence:.2f} MutRate:{eff_rate:.2f}"

                print(
                    f"Gen {gen:3d} | Best: {best_fit:.4f} | "
            ...
            best_raw_mse = (
                getattr(self.best_ever, "raw_mse", float("inf"))
                if self.best_ever
                else float("inf")
            )
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    pred_sq = pred.squeeze()
                    y_sq = y.squeeze()
                    current_corr = torch.corrcoef(
                        torch.stack([pred_sq.cpu(), y_sq.cpu()])
                    )[0, 1].item()
                    if (
                        not math.isnan(current_corr)
                        and current_corr > self.early_stop_corr
                    ):
                        print(
            ...
            if self.best_ever and best_raw_mse < 0.01:
                ...
                if (
                    hasattr(self, "confidence_tracker")
                    and self.confidence_tracker.generations_stable > 2
                ):
                    print(
            ...
            if (
                hasattr(self, "confidence_tracker")
                and self.confidence_tracker.confidence > 0.95
            ):
            ...
            if (
                hasattr(self, "confidence_tracker")
                and self.confidence_tracker.should_refine_only()
                and self.confidence_tracker.generations_stable > 8
            ):
                print(
            ...
            if self.visualizer is not None and self.best_ever is not None:
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        y_np = y.squeeze().cpu().numpy()
                        correlation = float(
                            np.corrcoef(pred_np.flatten(), y_np.flatten())[0, 1]
                        )
                ...
                for ind in self.population:
                    if self.use_adaptive_pruning:
                        ...
                        n_pruned, new_mse = adaptive_coefficient_pruning(
                            ind.model, fit_x, fit_y, prune_ratio=PRUNE_MSE_TOLERANCE
                        )
                    else:
            ...
            if gen < generations - 1:
                if self.confidence_tracker.should_refine_only():
                    ...
                    effective_mutation_rate = (
                        self.confidence_tracker.get_effective_mutation_rate(
                            self.mutation_rate
                        )
                    )
                    ...
                    if self.gradient_monitor is not None:
                        ...
                        if rspg_stats.get("is_stuck", 0.0) > 0.5:
                            ...
                            effective_mutation_rate = min(
                                0.9, effective_mutation_rate * 2.5
                            )
                    ...
                    self.select_and_reproduce(
                        x, y, diversity=diversity, mutation_rate=effective_mutation_rate
                    )
        ...
        if self.best_ever:
            ...
            if structure_good and mse_before > 0.01:
                ...
                print(
                    f"Structure LOCKED (corr={corr:.4f}). MSE={mse_before:.4f} - running intensive coefficient refinement..."
                )
                ...
                if mse_after < mse_before * 0.8:  # Improved by at least 20%
                    print(
                        f"  Phase 2 refinement: MSE {mse_before:.4f} -> {mse_after:.4f} ({steps} steps)"
                    )
                else:
                    # Refinement didn't help much, try output_proj only as fallback
                    print(
                        f"  Phase 2 refinement didn't improve enough ({mse_before:.4f} -> {mse_after:.4f})"
                    )
                    ...
                    if output_proj_params:
                        ...
                        with torch.no_grad():
                            ...
                            mse_fallback = F.mse_loss(
                                pred_fallback.squeeze(), y.squeeze()
                            ).item()

                        if mse_fallback < mse_before:
                            print(
                                f"  Fallback output-proj refinement: MSE {mse_before:.4f} -> {mse_fallback:.4f}"
                            )
                        else:
                            # Revert entirely
                            self.best_ever.model.load_state_dict(backup_state)
                ...
                if mse_after > mse_before * MSE_DEGRADATION_TOLERANCE:
                    logger.info(
                        f"  Refinement hurt (MSE {mse_before:.4f} -> {mse_after:.4f}), reverting..."
                    )
                    self.best_ever.model.load_state_dict(backup_state)
                else:
                    logger.info(
                        f"  Refinement: MSE {mse_before:.4f} -> {mse_after:.4f}"
                    )
        ...
        # Get best model
        best_model = (
            self.best_ever.model if self.best_ever else self.population[0].model
        )
        ...
        with torch.no_grad():
            ...
            corr = torch.corrcoef(torch.stack([pred.cpu(), y_eval.cpu()]))[0, 1].item()
...
def train_onn_hybrid(
    ...
    for cycle in range(cycles):
        ...
        # Get best from ES
        es_best = (
            trainer.best_ever.model
            if trainer.best_ever
            else trainer.population[0].model
        )
        ...
        continuous_params = [
            p
            for n, p in es_best.named_parameters()
