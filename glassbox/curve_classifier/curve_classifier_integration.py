...
def describe_curve_classifier_inference(x: np.ndarray) -> dict[str, object]:
    ...
    if x_arr.ndim == 1:
        ...
        raise ValueError(
            f"Expected x to be 1D, [N,1], or [N,D], got shape {x_arr.shape}"
        )
...
_cached_interpolators_by_signature: dict[tuple, tuple[object | None, object]] = {}
...
def validate_curve_classifier_checkpoint_metadata(
    ...
    *,
    ...
    if model_type is None:
        warnings.append(
            "missing model_type; falling back to state_dict architecture detection"
        )
    ...
    architecture_version = checkpoint.get("architecture_version") or model_config.get(
...
def _load_pytorch_classifier(
    model_path: Path, resolved_device: torch.device, cache_key: str
) -> nn.Module:
    ...
    if model_type == "cnn":
        ...
        model = CurveClassifierGLU(
            n_features=n_features, n_classes=n_classes, hidden=hidden_size
        )
    else:
        if "hidden" in model_config and model_config["hidden"] is not None:
            hidden_size = int(model_config["hidden"])
...
def _predict_pytorch(
    model: nn.Module, features: np.ndarray, metadata: dict, device: torch.device
) -> np.ndarray:
...
def _evaluate_demo_formula(formula: str, x: np.ndarray) -> np.ndarray:
    ...
    if y is None:
        raise ValueError(
            "formula could not be evaluated safely over the requested x range"
        )
    return np.asarray(y, dtype=np.float64)


def _prepare_curve_features(
    ...
    if end > 192:
        prepared[192:end] = np.sign(prepared[192:end]) * np.log1p(
            np.abs(prepared[192:end])
        )
...
def predict_operators(
    ...
    try:
        model = load_classifier(
            model_path, device=str(resolved_device), n_samples=n_samples
        )
        load_status["ok"] = True
    except Exception as e:
        print(
...
def _predict_operators_multi_input(
    ...
    def _interp_array_fp(arr: np.ndarray) -> tuple:
        ...
        return (
            ...
            ptr,
            sample_fp,
            float(np.nanmean(flat)) if flat.size else 0.0,
        )
    ...
    if interpolators is None:
        ...
        if linear_interp is None and nearest_interp is None:
            raise RuntimeError(
                "Failed to build interpolation models for multi-input prediction."
            )
        ...
        if len(_cached_interpolators_by_signature) > 8:
            _cached_interpolators_by_signature.pop(
                next(iter(_cached_interpolators_by_signature))
            )
...
def _build_result_dict(
    probs: np.ndarray, threshold: float, metadata: dict, cache_key: str
) -> dict[str, float]:
        ...
        for node in layer.nodes:
            ...
            if hasattr(selector, "logits") and hasattr(selector, "_type_end"):
                with torch.no_grad():
                    ...
                    periodic_prob = max(
                        predictions.get("sin", 0), predictions.get("cos", 0)
                    )
                    if periodic_prob >= threshold:
                        selector.logits.data[0] += (
                            periodic_prob * boost_factor
                        )  # unary type
                        n_biased += 1
