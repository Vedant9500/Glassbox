...
6. **N7:** ~~Pass weights into C++ MAD or document auto-δ unweighted-only.~~ **fixed** (2026-07-23): C++ `mad_scale` uses weighted median/MAD when `y_weights_` set.
...
| O17 | S5 | Weight-aware refine.h or route specialists through evolution residual_mse | Noise/sample_weight consistency | fixed |
...
| 2026-07-22 | S5-9/10 | **Weighted specialist refine + protected exact eval**: refine_frequencies/powers/periodic_rational WLS via sample_weight; exact Div protected; non-int power real domain; `eval_formula_exact` diag bind; `tests/test_s5_9_s5_10_refine_exact.py` |
| 2026-07-22 | Phase7 | **S1-11/S5-14/S5-16/S6-2**: export GlassboxRegressor+FPIPv2 alias; implicit mul + bare unary normalize; even Power print abs parity; legacy evaluate_fitness sets raw/weighted_mse+fitness_valid; bind elite_size/seed_fraction; `tests/test_phase7_api_polish.py` |
...
9. Phase 6.x P1 fixes complete (S3-1/2, S7-1, S8-1, S9-2, S10-5; S6-3 earlier)
