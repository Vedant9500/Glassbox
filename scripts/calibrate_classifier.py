...
def calibrate_existing_model(
    ...
    for i, name in enumerate(operator_classes):
        if i < len(isotonic_maps):
            ...
            print(
                f"  {name:15s}: calibrated range [{min(values):.3f}, {max(values):.3f}]"
            )

    # Evaluate after calibration (apply to predictions)
    print("\n--- After calibration ---")
    raw_probs = torch.sigmoid(
        all_logits / temperature if temperature else all_logits
    ).numpy()
    ...
    binary_preds = (
        calibrated_preds
        > (current_thresholds if current_thresholds is not None else 0.5)
    ).float()
...
def quick_calibration_test(
    ...
    for name, func, x_range in test_cases[:n_test]:
        y = func(x_range)
        try:
            result = predict_operators(
                x_range, y, model_path=model_path, threshold=0.1, device=str(device)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Calibrate an existing curve classifier"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Path to existing .pt checkpoint"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--no-retune-thresholds", action="store_true", help="Skip threshold retuning"
    )
    parser.add_argument(
