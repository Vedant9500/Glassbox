...
def _metrics_for_tier(
    tier: dict[str, Any], *, n: int = 160, seeds: list[int] = None
) -> dict[str, Any]:
    ...
    for seed in seeds:
        ...
        for name, y_clean in targets:
            ...
            noisy_r2 = 1.0 - float(np.mean((const - y_noisy) ** 2)) / max(
                float(np.var(y_noisy)), 1e-12
            )
            clean_r2 = 1.0 - float(np.mean((const - y_clean) ** 2)) / max(
                float(np.var(y_clean)), 1e-12
            )
    ...
    return {
        ...
        "n": len(rows),
        "median_outlier_fraction": float(
            np.median([r["outlier_fraction"] for r in rows])
        ),
        "median_residual_autocorr": float(
            np.median([r["residual_autocorr"] for r in rows])
        ),
        "noise_band_mode": max(
            set(r["noise_band"] for r in rows),
            key=lambda b: sum(1 for r in rows if r["noise_band"] == b),
        ),
        "false_confidence_rate_const": float(
            np.mean([1.0 if r["false_confidence_const"] else 0.0 for r in rows])
        ),
        "rows": rows,
    }
