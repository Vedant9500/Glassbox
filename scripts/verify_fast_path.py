...
def run_test(benchmark, model_path=None):
    ...
    try:
        ...
        elapsed = (
            float(time_match.group(1)) if time_match else (time.time() - start_time)
        )
...
def main():
    ...
    if args.benchmark:
        benchmarks = [
            b for b in BENCHMARKS if args.benchmark.lower() in b["name"].lower()
        ]
        if not benchmarks:
    ...
    print("=" * 80)
    print(
        f"{'BENCHMARK':<30} | {'STATUS':<6} | {'MSE':<10} | {'TIME':<8} | {'FORMULA'}"
    )
    print("-" * 80)
    ...
    for r in results:
        ...
        print(
            f"{r['name']:<30} | {status:<6} | {r['mse']:<10.6f} | {r['time']:<8.2f} | {f_disp}"
        )
