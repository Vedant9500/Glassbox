...
def _evaluate_run(
    ...
    *,
    ...
    screening = (getattr(est, "blackbox_diagnostics_", {}) or {}).get(
        "candidate_screening", {}
    )
    specialist = screening.get("specialist_screening")
    composition = (getattr(est, "blackbox_diagnostics_", {}) or {}).get(
        "specialist_composition_screening", {}
    )
    vault = (getattr(est, "blackbox_diagnostics_", {}) or {}).get(
        "specialist_vault", {}
    )
    top_pairs = specialist.get("top_pairs", []) if isinstance(specialist, dict) else []
    max_pair_score = max(
        (float(p.get("complementarity_score", 0.0)) for p in top_pairs), default=0.0
    )
    ...
    return {
        ...
        "specialist_segment_count": int((specialist or {}).get("segment_count", 0) or 0)
        ...
        "specialist_screening": specialist
        ...
        "specialist_vault_entry_count": int((vault or {}).get("entry_count", 0) or 0)
        if isinstance(vault, dict)
        else 0,
        "specialist_vault_composition_count": int(
            (vault or {}).get("composition_count", 0) or 0
        )
        if isinstance(vault, dict)
        else 0,
...
def run_phase0(*, quick: bool = False) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        try:
            baseline = _evaluate_run(
                case, enable_specialist_screening_diagnostics=False
            )
        ...
        mse_delta = (
            None
            if baseline_mse is None or phase0_mse is None
            else float(phase0_mse - baseline_mse)
        )
        r2_delta = (
            None
            if baseline_r2 is None or phase0_r2 is None
            else float(phase0_r2 - baseline_r2)
        )
        if case_error is not None or (
            mse_delta is not None
            and r2_delta is not None
            and (abs(mse_delta) > 1e-9 or abs(r2_delta) > 1e-9)
        ):
    ...
    summary["pass"] = bool(
        ...
        and (
            ...
            or summary["specialist_hits"]
            >= max(1, summary["specialist_expected_cases"] - 1)
        )
...
def run_phase4(*, quick: bool = False) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        if case["name"] == "composed_product_with_residual":
            ...
            if (
                formula
                and ("sin" in formula or "x1" in formula)
                and ("exp" in formula or "x3" in formula)
            ):
                phase4_hits += 1
...
def run_phase6(*, quick: bool = False) -> dict[str, Any]:
    cases = _phase0_cases()
    p6_names = {
        "phase6_nested_sin_cos",
        "phase6_sigmoid_gate",
        "phase6_damped_oscillation",
    }
...
def run_phase8(*, quick: bool = False) -> dict[str, Any]:
    ...
    for case in p8_cases:
        ...
        probe_candidates = [
            {
                ...
                "validation_r2": 0.80,
                "validation_mse": 0.02,
                "source": "probe_product",
            },
            {
                "formula": "0.1*x3",
                "validation_r2": 0.20,
                "validation_mse": 0.08,
                "source": "probe_residual",
            },
            {
                "formula": "x1*x2 + 0.1*x3",
                "validation_r2": 0.99,
                "validation_mse": 0.001,
                "source": "probe_combined",
            },
        ]
        ...
        vault_probe_diag["seeded_vault_candidate_count"] = sum(
            1
            for candidate in (seeded_candidates or [])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate specialist-layer phases against baseline behavior."
    )
    parser.add_argument(
        "--phase", type=int, default=0, help="Specialist phase to evaluate"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Run a smaller smoke subset"
    )
    parser.add_argument(
    ...
    if args.phase == 0:
        ...
        raise SystemExit(
            f"Only phase 0, 2, 3, 4, 5, 6, 7, 8, and 9 are implemented in this harness right now, got phase={args.phase}"
        )
